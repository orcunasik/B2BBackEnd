            ...
            using (SimpleContextDb context = new())
            {
                var result = from customer in context.Customers
                             select new CustomerDto
                             {
                                 ...
                                 Discount = context.CustomerRelationships.Where(p => p.CustomerId == customer.Id) != null
                                 ? context.CustomerRelationships.Where(p => p.CustomerId == customer.Id).Select(s => s.Discount).FirstOrDefault()
                                 : 0,
                                 PriceListId = context.CustomerRelationships.Where(p => p.CustomerId == customer.Id) != null
                                 ? context.CustomerRelationships.Where(p => p.CustomerId == customer.Id).Select(s => s.PriceListId).FirstOrDefault()
                                 :0,
                                 PriceListName = context.CustomerRelationships.Where(p => p.CustomerId == customer.Id) != null
